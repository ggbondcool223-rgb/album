...
class VideoEditLogic extends GetxController {
  ...
  void onInit() {
    ...
    if (args != null && args['albumId'] != null) {
      ...
      loadAlbum();
    } else if (args != null && args['videoImages'] != null) {
      final videoImages = args['videoImages'] as List<Map<String, dynamic>>;
      loadVideoImages(videoImages);
    }
    ...
    _saveHistory();
    // _startPhotoAutoSwitch() will be called after photos are loaded in loadAlbum() or loadVideoImages()
  ...
  Future<void> loadVideoImages(List<Map<String, dynamic>> videoImages) async {
    try {
      final now = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
      final photoList = <AlbumPhotoEntity>[];

      for (var entry in videoImages.asMap().entries) {
                              ...
                              if (background?.videoPath != null)
                                ClipRRect(
                                  ...
                                  child: SizedBox(
                                    ...
                                    child: TemplateVideoPlayer(
                                      videoPath: background!.videoPath!,
                                      fit: BoxFit.cover,
                                    ),
                                  ),
                                )
                              else if (background?.imagePath == null)
                                Center(
                                  child: Icon(
                                    Icons.videocam,
                                    color: Colors.white,
  ...
  Future<List<TemplateEntity>> _getVideoTemplates() async {
    final allTemplates = await db.getTemplates();
    return allTemplates.where((t) => t.type == 'video').toList();
  }
